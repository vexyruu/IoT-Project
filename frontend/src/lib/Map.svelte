<script lang="ts">
  ...
  export let onSelect: (aircraft: Aircraft) => void = () => {};
  ...
  let planeIcon: DivIcon;
  let selectedIcon: DivIcon;
  ...
  function syncMarkerIcons() {
    ...
    for (const [id, m] of markers) {
      m.setIcon(id === selectedIcao ? selectedIcon : planeIcon);
    }
  }
  ...
  $: selectedIcao, map && syncMarkerIcons();
